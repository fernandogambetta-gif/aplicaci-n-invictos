...
import { PaymentMethod, Sale, SaleAdjustment } from '../types';

interface SaleAdjustmentReceiptModalProps {
  sale: Sale;
...
const SaleAdjustmentReceiptModal: React.FC<SaleAdjustmentReceiptModalProps> = ({
  sale,
  adjustment,
  onClose,
}) => {
  ...
  const receiptNumber = useMemo(
    () => adjustment.id.replace(/^adj-/, '').slice(0, 18),
    [adjustment.id],
  );

  const fileName = useMemo(
    () =>
      `comprobante_${adjustment.type === 'exchange' ? 'cambio' : 'devolucion'}_invictos_${cleanFileName(
        receiptNumber,
      )}.pdf`,
    [adjustment.type, receiptNumber],
  );

  const title = adjustment.type === 'exchange' ? 'Comprobante de cambio' : 'Comprobante de devolución';
  ...
  const buildPdf = async () => {
    ...
    try {
      ...
      addText(`Venta original: ${sale.id.slice(-8)}`, 10, true);
      addText(`Fecha de venta: ${new Date(sale.timestamp).toLocaleString('es-AR')}`);
      addText(`Fecha del movimiento: ${new Date(adjustment.timestamp).toLocaleString('es-AR')}`);
      addText(`Cliente: ${sale.customerName || sale.receivable?.customerName || 'No informado'}`);
      addText(`Vendedor: ${sale.userName}`);
      ...
      addText(
        `${adjustment.returnedItem.quantity} × ${lineLabel(adjustment.returnedItem)} · $${money(
      ...
      y += 7;
      addText(
        'Este comprobante deja constancia del cambio/devolución registrado en INVICTOS y queda vinculado a la venta original.',
        8,
      );
    ...
    try {
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        await navigator.share({
          title,
          text: `${title} INVICTOS · venta ${sale.id.slice(-8)}`,
          files: [file],
        });
        return;
      }
      ...
      doc.save(fileName);
      setError('El navegador no permite compartir el PDF directamente. Se descargó el archivo para adjuntarlo.');
    } catch (e: any) {
      ...
      <div className="bg-white w-full h-[100dvh] sm:h-auto sm:max-h-[94dvh] sm:max-w-2xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <div className="shrink-0 px-4 sm:px-6 py-4 border-b border-slate-200 bg-emerald-50 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">N.º {receiptNumber} · Venta {sale.id.slice(-8)}</p>
          </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 sm:p-6 space-y-4">
          ...
          <section className="rounded-xl border border-slate-200 overflow-hidden">
            ...
            <div className="p-4 text-sm text-slate-700">
              <div className="font-bold text-slate-900">{adjustment.returnedItem.quantity} × {lineLabel(adjustment.returnedItem)}</div>
              <div className="mt-1">Valor reconocido: ${money(adjustment.returnedItem.totalAmount)}</div>
              <div className="mt-1 text-xs text-slate-500">
                {adjustment.returnedItem.returnToStock ? 'Reingresó al stock disponible.' : 'No reingresó al stock disponible.'}
              </div>
            </div>
          </section>
          ...
          {adjustment.replacementItem && (
            <section className="rounded-xl border border-indigo-200 overflow-hidden">
              ...
              <div className="p-4 text-sm text-slate-700">
                <div className="font-bold text-slate-900">{adjustment.replacementItem.quantity} × {lineLabel(adjustment.replacementItem)}</div>
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            {Math.abs(difference) >= 0.01 && (
              <div className="text-sm text-slate-600 mt-2">
                ...
                {adjustment.settlement.receiptNumber ? ` · Comprobante ${adjustment.settlement.receiptNumber}` : ''}
              </div>
            )}
          </section>

          <div className="text-sm text-slate-500">
            Registrado por <b className="text-slate-700">{adjustment.recordedByUserName}</b> el {new Date(adjustment.timestamp).toLocaleString('es-AR')}.
          </div>
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white p-3 sm:p-4" style={{ paddingBottom: 'calc(0.75rem + env(safe-area-inset-bottom, 0px))' }}>
...
export default SaleAdjustmentReceiptModal;
