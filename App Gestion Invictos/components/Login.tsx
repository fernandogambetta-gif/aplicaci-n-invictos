...
import { Users, Lock, ChevronRight, RefreshCw, AlertTriangle, Clock, Ban, Mail, Loader2 } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
                    ...
                    <div className="space-y-3">
                        {users.map(user => (
                         ...
                         {users.length === 0 && !isLoading && (
                             <p className="text-center text-red-400">Error: No hay usuarios. Revisa la conexión.</p>
                         )}
                    </div>
                )}
            </div>
            <div className="bg-slate-50 p-4 text-center text-xs text-slate-400 shrink-0">
                INVICTOS v1.1.0 Cloud
            </div>
        </div>
      </div>
    );
  ...
  // BLOCKED & PIN SCREENS REMAIN THE SAME, just updated imports and usage
      ...
      return (
          <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
              <div className="max-w-sm w-full bg-white rounded-2xl shadow-2xl p-8 text-center">
                  ...
                  <button onClick={() => { setSelectedUser(null); setRecoveryMode(false); }} className="w-full py-3 bg-slate-100 text-slate-600 rounded-lg font-bold hover:bg-slate-200 transition-colors">Volver</button>
              </div>
          </div>
      );
  }
        ...
        <div className="max-w-sm w-full bg-white rounded-2xl shadow-2xl overflow-hidden">
             <div className="p-8 text-center">
                ...
                <form onSubmit={handlePinSubmit} className="space-y-4">
                    <input 
                        type="password" inputMode="numeric" autoFocus maxLength={4}
                        ...
                        placeholder="••••" value={pin} onChange={(e) => { setPin(e.target.value); setError(''); }}
                    />
                    {error && <p className="text-red-500 text-sm font-medium animate-pulse">{error}</p>}
                    <div className="pt-4 flex gap-3">
                         <button type="button" onClick={() => { setSelectedUser(null); setPin(''); setError(''); }} className="flex-1 py-3 text-slate-600 font-medium hover:bg-slate-50 rounded-lg transition-colors">Atrás</button>
                         <button type="submit" disabled={pin.length < 1} className="flex-1 py-3 bg-indigo-600 text-white rounded-lg font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">Ingresar</button>
                    </div>
                </form>
             </div>
        </div>
