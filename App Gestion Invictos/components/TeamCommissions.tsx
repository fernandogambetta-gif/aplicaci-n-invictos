...
const TeamCommissions: React.FC<TeamCommissionsProps> = ({ sales, currentUser, onUpdate }) => {
  ...
  useEffect(() => {
    const load = async () => {
        const [usrs, conf] = await Promise.all([
            StorageService.getUsers(),
            StorageService.getConfig()
        ]);
        setUsers(usrs);
        setConfig(conf);
        setGlobalPercentage(conf.commissionPercentage);
    };
    load();
  }, [sales]);
  ...
  const handleSaveUserCommission = async (user: User) => {
  ...
  const handleAddUser = async () => {
      ...
      await StorageService.addUser(userToAdd);
  ...
  const handleMarkAsPaid = async (saleIds: string[]) => {
      await StorageService.markCommissionsAsPaid(saleIds);
      if (onUpdate) {
        onUpdate();
      }
  };
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full animate-fade-in-up overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <input type="text" className="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Ej: Juan Pérez" value={newUser.name} onChange={(e) => setNewUser({...newUser, name: e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">PIN de Acceso (4 dígitos)</label>
                        <div className="relative">
                            <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16}/>
                            <input type="text" inputMode="numeric" maxLength={4} className="w-full border border-slate-300 rounded-lg p-2 pl-9 focus:ring-2 focus:ring-indigo-500 focus:outline-none font-mono" placeholder="0000" value={newUser.pin} onChange={(e) => setNewUser({...newUser, pin: e.target.value})} />
                        </div>
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700 mb-1">Rol</label>
                        <select className="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white" value={newUser.role} onChange={(e) => setNewUser({...newUser, role: e.target.value as UserRole})}>
                            <option value="seller">Vendedor</option>
                            <option value="admin">Administrador</option>
                        </select>
                    </div>
                    <button onClick={handleAddUser} disabled={!newUser.name || newUser.pin.length < 4} className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-bold shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed mt-2">
                        Registrar Vendedor
                    </button>
                </div>
            </div>
...
const CommissionDetailModal: React.FC<CommissionDetailModalProps> = ({ user, usersList, config, sales, onClose, onPay, isOwner }) => {
    ...
    const totalSelectedAmount = unpaidSales.filter(s => selectedIds.has(s.id)).reduce((acc, s) => acc + calculateCommission(s), 0);
