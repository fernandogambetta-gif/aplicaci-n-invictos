...
const POS: React.FC<POSProps> = ({ products, onSaleComplete, currentUser }) => {
  ...
  const toNumber = (v: any, fallback = 0) => {
    const n = typeof v === 'string' ? Number(v.replace('%', '').replace(',', '.').trim()) : Number(v);
    return Number.isFinite(n) ? n : fallback;
  };

  const toRate = (v: any, fallback: number) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [conf, cats] = await Promise.all([StorageService.getConfig(), StorageService.getCategories()]);
        setConfig(conf && typeof conf === 'object' ? conf : { commissionPercentage: 5 });
        setCategories(Array.isArray(cats) ? cats : []);
      } catch (e) {
        console.error('Error loading config/categories', e);
  ...
  const addToCart = (product: Product) => {
    ...
    setCart((prev) => {
      ...
      if (existing) {
        if (existing.quantity >= stock) return prev; // Cannot add more than stock
  ...
  const updateQuantity = (productId: string, delta: number) => {
    setCart((prev) =>
      prev.map((item) => {
        ...
        return { ...item, quantity: newQuantity, subtotal: newQuantity * item.priceAtSale };
      })
    );
  };

  const cartSubtotal = cart.reduce((acc, item) => acc + toNumber(item.subtotal, 0), 0);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // ✅ Comisión robusta (evita NaN por datos corruptos)
      const configRate = toRate((config as any)?.commissionPercentage, 5);
      const userRate = toRate((currentUser as any)?.commissionPercentage, configRate);

      const itemsWithCommission = cart.map((item) => {
        const baseCommission = item.subtotal * (userRate / 100);
                ...
                <div className="mt-auto w-full flex justify-between items-end">
                  ...
                  <span
                    className={`text-xs px-2 py-1 rounded ${
                      stock < 3 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-600'
                    }`}
                  >
                    Stock: {stock}
                  </span>
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50 rounded-b-xl space-y-4">
            ...
            <div className="flex gap-2">
              <div className="flex bg-slate-100 rounded-lg p-1 border border-slate-200">
                ...
                <button
                  ...
                  className={`p-1.5 rounded ${
                    discountType === 'amount' ? 'bg-white shadow text-indigo-600' : 'text-slate-400 hover:text-slate-600'
                  }`}
                >
                  <DollarSign size={16} />
                </button>
          ...
          <div className="space-y-1 pt-2">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-slate-200">
              ...
              <span className="text-3xl font-bold text-indigo-600">${finalTotal.toFixed(2)}</span>
            </div>
          </div>
