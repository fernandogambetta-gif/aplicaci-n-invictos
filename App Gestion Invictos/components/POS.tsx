...
const POS: React.FC<POSProps> = ({ products, onSaleComplete, currentUser }) => {
  ...
  const handleCheckout = async () => {
    ...
    const itemsWithCommission = cart.map(item => {
      let commissionRate = config.commissionPercentage; 
      if (currentUser.commissionPercentage !== undefined) {
        commissionRate = currentUser.commissionPercentage; 
      }
    ...
    setCart([]);
    setDiscountValue('');
    await onSaleComplete(); // Refresh global data
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50 rounded-b-xl space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => setPaymentMethod('cash')} className={`text-xs font-medium py-2 rounded-lg border ${paymentMethod === 'cash' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300 text-slate-600'}`}>Efectivo</button>
              <button onClick={() => setPaymentMethod('card')} className={`text-xs font-medium py-2 rounded-lg border ${paymentMethod === 'card' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300 text-slate-600'}`}>Tarjeta</button>
              <button onClick={() => setPaymentMethod('transfer')} className={`text-xs font-medium py-2 rounded-lg border ${paymentMethod === 'transfer' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300 text-slate-600'}`}>Transf.</button>
            </div>
          </div>
