...
const POS: React.FC<POSProps> = ({ products, onSaleComplete, currentUser }) => {
  ...
  const handleCheckout = () => {
    ...
    // Calculate commissions with hierarchy: Product > User > Global
    const itemsWithCommission = cart.map(item => {
      ...
      if (product && product.commissionPercentage !== undefined) {
        commissionRate = product.commissionPercentage; // Product level overrides User/Global
      }
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-8rem)]">
          ...
          {filteredProducts.map(product => (
            ...
            >
              {product.commissionPercentage && (
                  <span className="absolute top-2 right-2 bg-indigo-100 text-indigo-700 text-[10px] font-bold px-1.5 py-0.5 rounded-full">
                      ★ {product.commissionPercentage}%
                  </span>
              )}
      ...
      <div className="w-full lg:w-96 bg-white rounded-xl shadow-lg border border-slate-200 flex flex-col h-full">
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50 rounded-b-xl space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-600 mb-2">Método de Pago</label>
            <div className="grid grid-cols-3 gap-2">
              <button 
                onClick={() => setPaymentMethod('cash')}
                className={`text-sm py-2 rounded-lg border ${paymentMethod === 'cash' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300'}`}
              >
                Efectivo
              </button>
              <button 
                onClick={() => setPaymentMethod('card')}
                className={`text-sm py-2 rounded-lg border ${paymentMethod === 'card' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300'}`}
              >
                Tarjeta
              </button>
              <button 
                onClick={() => setPaymentMethod('transfer')}
                className={`text-sm py-2 rounded-lg border ${paymentMethod === 'transfer' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white border-slate-300'}`}
              >
                Transf.
              </button>
            </div>
          ...
          <div className="flex justify-between items-center pt-2">
            ...
            <span className="text-3xl font-bold text-indigo-600">${cartTotal}</span>
          </div>
